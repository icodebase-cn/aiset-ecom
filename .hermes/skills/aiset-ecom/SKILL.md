...
name: aiset-ecom
description: |
  电商主图、详情页 AI 视觉创作 + 文案设计 SOP。
  图像生成：输入产品图和需求，生成商品主图、Amazon/Shopify PDP、社媒推广图、直播间场景图。
  文案设计：输入产品资料、主图初稿、资质边界和平台要求，输出可交给设计师执行的图文案方案：画面内容、图内文案、设计说明、合规自审。
  触发词包括：主图文案、详情页文案、电商文案、商品文案、listing、产品页面、转化率、点击率、合规审查、广告法、平台审核、淘宝、天猫、京东、拼多多、抖音小店、设计师执行稿、画面+文案+设计说明、visual strategy、image prompt writing、product/marketing image direction、direct text-to-image generation。
---
...
直接生图使用 apimart.ai 图像生成接口（GPT-Image-2，异步轮询模式）。优先在 `.claude/skills/aiset-ecom/` 放 `.env`，不要把真实 API key 写进仓库：
...
> 适用于需要图内文案、设计执行稿、合规审查的场景。输出每张图的**画面内容、图内文案、设计说明**。

### 何时使用视觉文案模式

用户提到以下任一需求时切换到此模式：

- 主图文案、详情页文案、电商文案、商品文案、listing
...
2. **先过用户确认门，再写最终稿。** 合规版必卖理由必须让用户确认后，才能进入主图和详情页。
...
建议补充：目标人群画像、竞品链接/截图、产品实拍图、品牌调性/禁用词/必须出现的免责声明。
...
### Step 3：主图 5 张（用户确认后执行）

每张主图只输出三项：**画面、图内文案、设计说明**。图内文案每张不超过 5 行。
...
按产品需要选择模块，每个模块仍然只输出：画面、图内文案、设计说明。每模块不超过 6 行文字。
...
- 用户没有资质证据却要求写"改善、调理、修复、治疗、增强"等表达：必须拒绝并给安全替代。
...
*aiset-ecom Skill — 视觉创作 + 文案设计双模式，先过合规线，再谈转化率。*
